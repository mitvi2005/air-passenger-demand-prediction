...
DATA_PATH = os.path.join(BASE_DIR, "new_air_passangers.csv")
...
@app.route('/')
def home():
    return render_template("index.html", stats=get_model_stats())


@app.route('/information')
def information():
    return render_template("information.html", stats=get_model_stats())


@app.route('/predictor')
def predictor():
    return render_template("predictor.html", stats=get_model_stats(), show_result=False)


@app.route('/about')
def about():
    return render_template("about.html", stats=get_model_stats())
...
def api_historical():
    try:
        df = pd.read_csv(DATA_PATH).drop_duplicates().sort_values(['year', 'month'])
...
def predict():
    try:
        ...
        data = pd.DataFrame({'month': [month], 'year': [year]})
        prediction = model.predict(data)
        pred_value = round(float(prediction[0]), 0)

        # Save new data (optional for production)
        new_data = pd.DataFrame({
            'Passengers': [pred_value],
            'month': [month],
            'year': [year]
